...
EncryptedNumber::EncryptedNumber(PaillierPublicKey* public_key, int ciphertext,
                                 int exponent) {
...
EncryptedNumber EncryptedNumber::operator+(int other) {
...
// Multiplying
EncryptedNumber EncryptedNumber::operator*(EncodedNumber& other) {
    int product = this->_raw_mult(other.encoding);
...
// Division
EncryptedNumber EncryptedNumber::operator/(int scalar) {
...
int EncryptedNumber::ciphertext(bool be_secure) {
...
EncryptedNumber EncryptedNumber::decrease_exponent_to(int new_exp) {
    ...
    EncryptedNumber multiplied =
        this->operator*((int)pow(BASE, this->exponent - new_exp));
...
void EncryptedNumber::obfuscate() {
    int r = this->public_key->get_random_lt_n();
    int r_pow_n = powmod(r, this->public_key->n, this->public_key->nsquare);
...
EncryptedNumber EncryptedNumber::_add_encoded(EncodedNumber& encoded) {
    ...
    int encrypted_scalar = a.public_key->raw_encrypt(b.encoding, 1);
    int sum_ciphertext = a._raw_add(a.ciphertext(false), encrypted_scalar);
...
EncryptedNumber EncryptedNumber::_add_encrypted(EncryptedNumber& other) {
    ...
    int sum_ciphertext = a._raw_add(a.ciphertext(false), b.ciphertext(false));

    return EncryptedNumber(a.public_key, sum_ciphertext, a.exponent);
}

EncryptedNumber EncryptedNumber::_add_scalar(int scalar) {
...
int EncryptedNumber::_raw_add(int e_a, int e_b) {
    return e_a * e_b % this->public_key->nsquare;
}

int EncryptedNumber::_raw_mult(int plaintext) {
    ...
    if (this->public_key->n - this->public_key->max_int <= plaintext) {
        int neg_c = invert(this->ciphertext(false), this->public_key->nsquare);
        int neg_scalar = this->public_key->n - plaintext;
